...
const S = {
  card: { background:'var(--bg-200)', border:'1px solid var(--border-200)', borderRadius:'var(--r-xl)', padding:32, boxShadow:'var(--shadow-xl)' },
  ...
  back: { width:'100%', marginTop:8, padding:10, color:'var(--text-400)', fontSize:13, background:'none', border:'none', cursor:'pointer', fontFamily:'var(--font-ui)' },
};

const HAS_ENV = !!(process.env.NEXT_PUBLIC_TG_API_ID && process.env.NEXT_PUBLIC_TG_API_HASH);

function Bar({ step, total }) {
  return (
    <div style={{ display:'flex', gap:5, marginBottom:28 }}>
      {Array.from({ length: total }).map((_,i) => (
        <div key={i} style={{ flex:1, height:3, borderRadius:99, background: i<=step ? 'var(--brand)' : 'var(--border-200)', transition:'background 400ms' }}/>
      ))}
    </div>
  );
}
...
function Btn({ children, loading, ...p }) {
  return (
    <button {...p} className="btn-primary"
      ...
      {loading
        ? <><span style={{ width:16, height:16, border:'2px solid rgba(255,255,255,0.3)', borderTopColor:'#fff', borderRadius:'50%', display:'inline-block', animation:'spin 0.8s linear infinite' }}/> Loading...</>
        : children}
    </button>
  );
}

// Only shown when no env vars configured
function StepCreds({ onNext }) {
  ...
  return <>
    ...
    <p style={{ color:'var(--text-300)', fontSize:13, marginBottom:22 }}>
      Get credentials at <a href="https://my.telegram.org/apps" target="_blank" rel="noopener noreferrer">my.telegram.org</a>
    </p>
    <Input label="API ID" type="number" placeholder="12345678" value={apiId} onChange={e=>setApiId(e.target.value)} hint="Found in API development tools"/>
    <Input label="API Hash" type="text" placeholder="0123456789abcdef..." value={apiHash} onChange={e=>setApiHash(e.target.value)}/>
...
function StepPhone({ onNext, onBack }) {
  ...
  async function go() {
    const p = phone.trim();
    if (!p) return toast.error('Enter your phone number');
    setLoading(true);
    try {
      const r = await sendCode(p);
      onNext({ phone: p, phoneCodeHash: r.phoneCodeHash });
    } catch(e) {
  ...
  return <>
    ...
    {!HAS_ENV && <button style={S.back} onClick={onBack}>Back</button>}
  </>;
}
...
function StepOTP({ data, onPassword, onBack }) {
  ...
  return <>
    ...
    />
    <Btn onClick={go} loading={loading}>Verify</Btn>
...
function StepPassword({ onBack }) {
  ...
  return <>
    ...
    <p style={{ color:'var(--text-300)', fontSize:13, marginBottom:22 }}>Enter your Telegram cloud password.</p>
    <div style={{ position:'relative', marginBottom:16 }}>
      ...
      <input type={show?'text':'password'} value={pw}
        ...
        placeholder="Enter password" className="input" style={{ paddingRight:44 }}/>
...
export default function LoginFlow() {
  const [step, setStep] = useState(HAS_ENV ? 1 : 0);
  const [data, setData] = useState({});
  const visualStep = HAS_ENV ? step - 1 : step;
  const total = HAS_ENV ? 3 : 4;

  return (
    <div style={S.card} className="a-up">
      <Bar step={visualStep} total={total} />
      {step === 0 && <StepCreds onNext={() => setStep(1)} />}
      {step === 1 && <StepPhone onNext={d => { setData(p=>({...p,...d})); setStep(2); }} onBack={() => setStep(0)} />}
      {step === 2 && <StepOTP data={data} onPassword={() => setStep(3)} onBack={() => setStep(1)} />}
      {step === 3 && <StepPassword onBack={() => setStep(1)} />}
    </div>
  );
}
